...
describe("YieldMindCore", function () {
  ...
  beforeEach(async function () {
    ...
    // Deploy strategies - owner is also YieldMindCore owner so harvest works
    ...
    // Transfer strategy ownership to YieldMindCore so harvest() works
    await strategy1.transferOwnership(await yieldMind.getAddress());
    await strategy2.transferOwnership(await yieldMind.getAddress());
  ...
  describe("Strategy Management", function () {
    it("Should add a new strategy", async function () {
      // Remove one existing strategy first to make room
    ...
    it("Should update strategy allocation", async function () {
      // Must keep total <= 10000, so 5000+5000 = 10000
      await yieldMind.updateStrategyAllocation(await strategy1.getAddress(), 5000);
      const info = await yieldMind.getStrategyInfo(await strategy1.getAddress());
      expect(info.allocationBps).to.equal(5000);
    });
  ...
  describe("Deposits", function () {
    it("Should deposit tokens correctly", async function () {
      ...
      expect(sharesAmount).to.equal(DEPOSIT_AMOUNT - MINIMUM_SHARES); // First deposit minus dead shares
    });
    ...
    it("Should emit Deposit event", async function () {
      await expect(yieldMind.connect(user1).deposit(DEPOSIT_AMOUNT))
        ...
        .withArgs(user1.address, DEPOSIT_AMOUNT, DEPOSIT_AMOUNT - MINIMUM_SHARES);
    });
  ...
  describe("Withdrawals", function () {
    ...
    it("Should withdraw tokens correctly", async function () {
      ...
      expect(balanceAfter - balanceBefore).to.equal(DEPOSIT_AMOUNT - MINIMUM_SHARES);
    });
    ...
    it("Should emit Withdraw event", async function () {
      ...
      await expect(yieldMind.connect(user1).withdraw(shares))
        ...
        .withArgs(user1.address, DEPOSIT_AMOUNT - MINIMUM_SHARES, shares);
    });
